...
class Archiver():
    ...
    def __init__(self, db_lib, config:dict):
        ...
        self.archiveted_file_rec = Archivated_file()
        self.file_pack_record = File_package()
    ...
    def archive(self, file_path, owner):
        ...
        it need file path to file which will be archivated
    ...
    def _assign_tsa_info(self):
        ...
        self.file_pack_record.TimeStampingAuthority = self.archivation_config['TSA_info']['url']
    ...
    def _validate_certificates(self):
        ...
        with open(path_crl, 'rb') as f: #NOTE: comment if you dont want to use CRL
            crl_s = f.load() #NOTE: comment if you dont want to use CRL
        validate_certificate(crl_s,path_tsa_ca_pem) #NOTE: comment if you dont want to use CRL
        logger.debug("signing cert valid")


    def _transfer_file(self, file_path):
        if self.archivation_config["remote_access"] == False:
            logger.info("[archivation] starting to transfer local file to archivation storage")
            (self.archiveted_file_rec.PackageStoragePath, 
            self.archiveted_file_rec.OriginFileHashSha512) = self._get_file(file_path)
        else:
            logger.info("[archivation] starting to transfer remote file to archivation storage")    
            (self.archiveted_file_rec.PackageStoragePath, 
            self.archiveted_file_rec.OriginFileHashSha512) = self._get_remote_file(file_path)
    ...
    def _make_ts0(self):
        ...
        ts0 = self._timestamp_data(self.archiveted_file_rec.OriginFileHashSha512, 'timestamp0')
    ...
    def _make_package0(self):
        ...
        self.archiveted_file_rec.Package0HashSha512 = hash_file(sha512, package0_tar_path)
    ...
    def _sign_package(self):
        ...
        signature = self.__make_signature(self.archiveted_file_rec.Package0HashSha512)
        sig_path = store_signature(self.archiveted_file_rec.PackageStoragePath, signature)
        logger.info("[archivation] signature stored in directory witch package0")
        self.archiveted_file_rec.SignatureHashSha512 = hash_file(sha512, sig_path)
    ...
    def _make_ts1(self):
        ...
        ts1 = self._timestamp_data(self.archiveted_file_rec.SignatureHashSha512, 'timestamp1')
        self.file_pack_record.IssuingDate = rfc3161ng.get_timestamp(ts1)
        self.archiveted_file_rec.ExpirationDateTS = self._get_expiration_date(ts1)    
        logger.info("[archivation] timestamp recieved succesfully")

    def _store_used_cert_files(self):
        logger.info("[archivation] storing used certificate files and available crls to archive directory")
        ...
        copy_file_to_dir(path_crl, dir_path, "signing_cert_crl.crl")#NOTE: comment if you dont want to use CRL
    ...
    def _make_final_package(self):
        logger.info("[archivation] creating final tar package with archive file")
        final_tar_path = self._make_tar_package_from_dir_content(
            self.archiveted_file_rec.PackageStoragePath,
            'Package1.tar'
        )
        self.file_pack_record.PackageHashSha512 = hash_file(sha512, final_tar_path)
        logger.info("[archivation] final package created, archivation successfull")

    def _insert_db_record(self):
        logger.info("[archivation] writing records with gathered data to database")
        try:
            ...
            logger.debug("deleting created archivated file")
    ...
    def _timestamp_data(self, fhash, ts_name):
        ...
        ts = get_timestamp(
            self.archivation_config['TSA_info'], 
            fhash
        )
        logger.debug("[archivation] timestamp recieved successfully, storing timestamp to archive direcotry")
    ...
    def _get_file(self, file_path):
        logger.debug(
            "[archivation] getting hash of file on path %s", 
            str(file_path)
        )
        ...
        self.dst_file_path = copy_file_to_dir(
                file_path, 
                copy_dir_path, 
                self.archiveted_file_rec.FileName
            ) 
        logger.debug(
            "[archivation] file copied, path: %s",
        ...
        try:
            with closing(
                get_sftp_connection(self.archivation_config["remote_access"])
                    ...
                    self.dst_file_path = self._copy_remote_file_to_archive(sftp_connection, file_path, copy_dir_path)
                    logger.debug(
                        "[archivation] remote file copied to destination archive: %s",
                        str(self.dst_file_path)
                    )
        except Exception as e:
            error_count += 1
            if error_count < 5:
                logger.warning("Exception occured during SFTP transfer, retrying session", exc_info= True)
            else:
                logger.exception("Failed to gather remote file", exc_info=True, stack_info= True)
                raise e
        return copy_dir_path, origin_hash
    ...
    def _get_expiration_date(self, ts):
        ...
        logger.debug("[archivation] expiration length of timestamp was set to %s", str(years))
        return time.replace(year= time.year+years)

    def _get_file_name(self, origin_file_path):
        head, tail = ntpath.split(origin_file_path)
        logger.debug(
            "[archivation] getting file name from path, splited path head: %s, tail(should be name) %s ",
            str(head), str(tail)
            )
    ...
    def _make_tar_package_from_dir_content(self, dir_path, package_name):
        ...
        logger.debug("[archivation] creating tar package on path: %s", str(tar_file_path))
    ...
    def _copy_remote_file_to_archive(self, connection_sftp, file_path_to_copy, dst_dir):
        dst = os.path.join(
                dst_dir,
                self.archiveted_file_rec.FileName
            )
    ...
    def _validate_data_transfer(self, hash_origin, dst_file_path):
        ...
        logger.debug(
            "[archivation] hash of origin file: %s \n hash of copy: %s",
             str(hash_origin), str(hash_copy)
        )
        if hash_origin != hash_copy:
            logger.warning(
                "[archivation] hashes of files do not match, file transfer wasnt succesfull"
                )
            raise FileTransferWasntSuccesfullError("[archivation] hashes of files do not match, file transfer wasnt succesfull")
