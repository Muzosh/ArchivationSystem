...
class Archivated_file(object):
    ...
    def map_columns(self):
        ...
        self.OriginFileHashSha512 = self.c_data['OriginFileHashSha512'] #BYTES 
    ...
    def validate_columns(self):
        ...
        if not isinstance(self.OriginalFilePath, str):
            logger.exception("[db_record] wrong type of OriginalFilePath: %s",
                str(type(self.OriginalFilePath))
                )
            errornous_columns = True
        if not isinstance(self.PackageStoragePath, str):
            logger.exception("[db_record] wrong type of PackageStoragePath: %s",
                str(type(self.PackageStoragePath))
                )
            errornous_columns = True 
        if not isinstance(self.OriginFileHashSha512, bytes): 
            logger.exception("[db_record] wrong type of OriginFileHashSha512: %s",
        ...
        if not isinstance(self.SigningCert, bytes): 
            logger.exception("[db_record] wrong type of SigningCert: %s",
                str(type(self.SigningCert))
                )
            errornous_columns = True 

        if not isinstance(self.SignatureHashSha512, bytes):
            logger.exception("[db_record] wrong type of SignatureHashSha512: %s",
                str(type(self.SignatureHashSha512))
                )
            errornous_columns = True 

        if not isinstance(self.Package0HashSha512, bytes): 
            logger.exception("[db_record] wrong type of Package0HashSha512: %s",
                str(type(self.Package0HashSha512))
                )
            errornous_columns = True 

        if not isinstance(self.ExpirationDateTS, datetime):
            logger.exception("[db_record] wrong type of ExpirationDateTS: %s",
