...
from common.utils import (
    store_ts_data, get_timestamp, verify_timestamp, 
    create_new_dir_in_location, copy_file_to_dir,
    hash_file, create_tar_file_from_dir, 
    get_certificate, get_current_crl, validate_certificate
)
from common.exceptions import (
    FileNotInDirectoryError, DigestsNotMatchedError, 
    TimestampInvalid
)
...
class Retimestamper():
    ...
    def retimestamp(self, file_id):
        ...
        of archivated file which will be retimestamped
        """
        storage_dir, actual_package_hash, packID = self._verify_existing_package(file_id)
        ts_new = self._create_new_timestamp(
            storage_dir, actual_package_hash
        )
        tar_path = create_tar_file_from_dir(
            storage_dir, os.path.join(storage_dir, 'PackageF{}.tar'.format(packID))
        )
        self._fill_package_record(file_id, ts_new, tar_path)


        logger.info("[retimestamping] updating archivated record with latest expiration date")
        self.db_lib.update_expiration_date_specific_record(
            file_id, 
            self._get_expiration_date(ts_new)
        )
        logger.info("[retimestamping] inserting file package record into database")
        self.db_lib.create_new_record_FilePackages(
            self.file_pack_record
        )
    ...
    def _verify_existing_package(self, file_id):
        ...
        logger.debug("[retimestamping] getting arichivated file record from db")
        arch_f = self.db_lib.get_specific_ArchivatedFile_record_by_FileId(file_id)
        logger.debug("[retimestamping] getting latest package file record from db")
        latest_file_p = self.db_lib.get_FilePackages_records(file_id, latest=True)
        storage_dir = arch_f.PackageStoragePath
        logger.debug("[retimestamping] getting data from package")
        ts, data, tar_package_path = self._get_ts_data_from_package(storage_dir)          
        actual_package_hash = hash_file(sha512, tar_package_path)

        logger.debug("[retimestamping] verifying latest package hashes with db record")
        self._verify_final_package_hashes(
            latest_file_p.PackageHashSha512,
            actual_package_hash
        )
        logger.debug("[retimestamping] verifying latest timestamp")
        val = verify_timestamp(ts, data, self.config['TSA_info']) 
        if val is not True: 
            logger.exception("[retimestamping] Last timestamp of package in path %s is invalid", str(storage_dir))
            raise TimestampInvalid('Last timestamp of package is invalid') 
        logger.info("[retimestamping] last package timestamp has been succesfuly validated")
    ...
    def _create_new_timestamp(self, storage_dir, actual_package_hash):
        ...
        logger.info("[retimestamping] storing timestamp to storage directory..")
    ...
    def _fill_package_record(self, file_id, ts_new, tar_path):
        logger.info("[retimestamping] Filling new file package record with gathered data")
        self.file_pack_record.ArchivatedFileID = file_id
        self.file_pack_record.TimeStampingAuthority = self.config['TSA_info']['url']
    ...
    def _get_ts_data_from_package(self, dir_path):
        ...
        package_name = list(filter(lambda x: x.startswith('Package'), files_in_dir))
        if len(package_name) == 0:
            logger.exception(
                "[retimestamping] unable to find package in directory: %s", 
                str(dir_path)
            )
            raise FileNotInDirectoryError('directory doesnt have files which shoud be there.') 
        ...
        logger.debug(
            "[retimestamping] needed data from tar package have been succesfuly retrieved"
        )           
    ...
    def _get_timestamped_file_name(self, names):
        ...
        if len(package)!=0:
            ...
            return package[0]
        logger.exception(
            "[retimestamping] Tar package doesnt have files which shoud be there. Tar Content: %s", 
            str(names)
        )
        raise FileNotInDirectoryError('Tar package doesnt have files which shoud be there.') 
    ...
    def _store_used_cert_files(self, storage_dir):
        ...
        logger.debug("[retimestamping] certificate files stored in dir of new package")

    def _hash_file_within_tar(self, hash, tarf, names):
        f = tarf.extractfile(
            self._get_timestamped_file_name(names)
        )
        file_hash = hash()
        buffer = f.read(8192)
        while buffer != b'':
    ...
    def _read_timestamp_from_tar(self, names, tarf):
        logger.debug("[retimestamping] trying to read timestamp from opened tar file")
        ts_name = list(filter(lambda x: x.startswith('timestamp'), names))
        if len(ts_name) == 0:
            logger.debug("[retimestamping] unable to find timestamp file in tarfile")
            raise FileNotInDirectoryError('[retimestamping] unable to find timestamp file in tarfile')
    ...
    def _verify_final_package_hashes(self, hash_db, hash_pack):
        ...
        if hash_db != base64.b64encode(hash_pack):
            logger.warning(
                "[retimestamping] hashes of files do not match, verification of last timestamp wasnt succesffull"
                )
            raise DigestsNotMatchedError("[retimestamping] hashes of files do not match, verification of last timestamp wasnt succesffull") 
