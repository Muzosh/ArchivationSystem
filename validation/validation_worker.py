...
class Validation_Worker():
    ...
    def validate(self, body):
        ...
        WrongTask Exception
        """
        logger.info("[validation_worker] recieved task with body: %s", str(body))
        ...
        with Mysql_connection(self.db_config) as db_connection:
            ...
            logger.info("[validation_worker] executing validation of file: %s", str(files_info))
    ...
    def _parse_message_body(self,body):
        ...
        if not body["task"] == "Validation":
            ...
            raise exceptions.WrongTaskError("task is not for this worker")
...
def run_worker(config):
    ...
    arch_worker = Validation_Worker(config)
    arch_worker.run()
