...
from common.utils import (
    hash_file, load_data, verify_timestamp, 
    validate_certificate, validate_signature,
    get_certificate, get_sftp_connection, get_remote_hash
)
from common.exceptions import (
    FileNotInDirectoryError, WrongPathToArchivatedFileError, 
    WrongTaskError, DigestsNotMatchedError, TimestampInvalid,
    OriginalFileNotValidError, ArchivedFileNotValidError,
    CertificateNotValidError, UnableToGetRemoteFileDigest
)
...
class Validator():
    ...
    def validate(self, file_info, recipients):
        """
        This will validate archivated package. It needs
        file info which could be file ID or name and owner
        to determine which archivated file will be validated.
        It needs list of recipients email which will be informed
        about validation result
        """
        archivated_file_rec = self._get_archive_record(file_info)
        file_packages = self._get_all_sorted_filepackage_records(
            archivated_file_rec.FileID
        )
        tar_path = self._get_file_path_from_dir(
            archivated_file_rec.PackageStoragePath, 
            'Package'
        )
        try:
            with TemporaryDirectory() as temp_dir:
                logger.debug(
                    "[validation] temporary directory for verification created at path %s", str(temp_dir)
                    )
        ...
        except OriginalFileNotValidError:#cus tom exceptoion for failed validation of this file
            result = "archivated file with id: {} isnt valid in remote location,\
                 archivated file is still valid".format(
                str(archivated_file_rec.FileID)
            )
            logger.warning(result)
             
        except ArchivedFileNotValidError:#cus tom exceptoion for failed validation of this file
            result = "archivated file with id: {}\
                 isnt valid in archive location".format(
                str(archivated_file_rec.FileID)
            )
            logger.warning(result)
            
        
        self._send_results(result ,recipients, archivated_file_rec.FileName)
        return 'OK'

    def _get_archive_record(self, file_info):
        logger.info("[validation] getting archivated file record from db")
        logger.debug("[validation] archivated file info from task %s", str(file_info))
        if file_info.isdigit():
            return self.db_lib.get_specific_ArchivatedFile_record_by_FileId(int(file_info))
        elif isinstance(file_info, tuple):
            return self.db_lib.get_fileID_ArchivatedFile_rec(file_info[1], file_info[0])
        logger.warning("[validation] Wrong task format for validation")
        raise WrongTaskError("Wrong task format for validation")

    def _get_all_sorted_filepackage_records(self, file_id):
        logger.info("[validation] getting all file package records associated with archivated file")
        file_recs = self.db_lib.get_FilePackages_records(file_id)
        file_recs.sort(key=lambda x: x.IssuingDate, reverse=True)
        return file_recs

    def _validate_packages(self, tar_path, temp_dir, file_packages, archivated_file_rec):
        num = 0
        validation_complete = False
        logger.info("[validation] validating archivated file")
        while(validation_complete is not True):
            logger.info(
                "[validation] validateing package from path: %s", 
                str(tar_path)
            )
            if "Package1.tar" in tar_path:
                try:
                    ...
                    logger.info("[validation] validation of initial package complete")
                except(
                    TimestampInvalid, DigestsNotMatchedError, 
                    InvalidSignature, FileNotInDirectoryError, 
                    CertificateNotValidError
                    ):
                    logger.warning("[validation] package invalid")
                    raise ArchivedFileNotValidError("[validation] package invalid")
            elif "PackageF" in tar_path:
                try:
                    tar_path = self._validate_package(
                        tar_path, temp_dir, file_packages[num]
                    )
                    logger.info("[validation] validation of onion package complete")
                except (
                    TimestampInvalid, DigestsNotMatchedError, 
                    FileNotInDirectoryError,CertificateNotValidError
                    ):
                    logger.warning("[validation] package invalid")
                    raise ArchivedFileNotValidError("[validation] package invalid")
                ...
                raise WrongPathToArchivatedFileError("Wrong file for validation")
            num += 1
        return result   

    def _validate_package(self, tar_path, temp_dir, file_package):
        logger.debug("[validation] validating package %s hashes with db", str(tar_path))
        self._verify_package_hashes(
            file_package.PackageHashSha512,
            hash_file(sha512, tar_path)
        )
        ...
        self._verify_certificate_with_crl(pack_path, 'tsa_cert_crl.crl', 'tsa_ca.pem')   
        return self._get_file_path_from_dir(pack_path, 'Package')

    def _validate_initial_pacakge(self, package_path, temp_dir, file_package, archivated_file_rec):
        logger.debug("[validation] getting package1 hash and verifing it")
        hash_package1 = hash_file(sha512, package_path)
        self._verify_package_hashes(file_package.PackageHashSha512, hash_package1)
        logger.debug("[validation] extracting package1")
        extract_path = self._extract_tar_to_temp_dir(package_path, 'Package1', temp_dir)
        ...
        self._verify_package_hashes(archivated_file_rec.Package0HashSha512, hash_package0)

        logger.debug("[validation] verifying signature and its timestamp")
        hash_signature = self._verify_signature(
            extract_path, hash_package0, 
            archivated_file_rec.SignatureHashSha512
        )
        ...
        logger.debug("[validation] verifying archivated file package hash with hash from database")
        hash_archivated_file = hash_file(
            sha512, 
            os.path.join(extrack_pack0_path, archivated_file_rec.FileName)
        )
        self._verify_package_hashes(
            archivated_file_rec.OriginFileHashSha512, 
            hash_archivated_file
        )
        
        self._verify_timestamp(extrack_pack0_path, 'timestamp0', hash_archivated_file)

        logger.debug("[validation] verifying certificates")
        self._verify_certificate_with_crl(extract_path, 'tsa_cert_crl.crl', 'tsa_ca.pem')
        self._verify_certificate_with_crl(extract_path, 'signing_cert_crl.crl', 'signing_cert.pem') #NOTE: comment if you dont want to use CRL
        
        logger.debug("[validation] verifying archivated file from archive storage with original storage")
        
        self._verify_original_file(
            archivated_file_rec.OriginFileHashSha512,
            archivated_file_rec.OriginalFilePath
        )
        return True    
    ...
    def _send_results(self, result, recipients, file_name):
        ...
        msg = MIMEMultipart()
        msg['Subject'] = 'Verification of file validity for: {}'.format(file_name)
        ...
        with smtplib.SMTP_SSL(
            ...
            smtp_serv.login(sender_mail, self.config['contact']['sender_password'])
    ...
    def _extract_tar_to_temp_dir(self, tar_path, tar_name, temp_dir_path):
        ...
        extract_path = os.path.join(
                    temp_dir_path, tar_name
                )
        logger.debug(
            "[validation] extracting tar package from path: %s to temp dir: %s", 
            tar_path,
            extract_path
        )
    ...
    def _verify_package_hashes(self, hash_db, hash_pack):
        logger.debug(
            "[validation] hash from db: %s \n hash of origin file: %s",
             str(hash_db), str(hash_pack)
        )
        if hash_db != base64.b64encode(hash_pack):
            logger.warning(
                "[validation] hashes of files do not match, validation wasnt succesffull"
                )
            raise DigestsNotMatchedError("[validation] hashes of files do not match, validation wasnt succesffull")
    ...
    def _verify_package_timestamp(self, pack_path):
        ...
        logger.debug(
            "[validation] getting hash of package to verify from path %s", str(data_path)
        )
    ...
    def _verify_signature(self, extract_path, signed_data, signature_hash_db):
        ...
        cert_path = self._get_file_path_from_dir(certs_folder_path, 'signing_cert.pem')
        logger.debug("[validation] loading certificate from path %s", str(cert_path))
    ...
    def _verify_timestamp(self, dir_path, file_name, data):
        ...
        if not (verify_timestamp(ts, data, self.config['TSA_info'])):
            logger.exception("[validation] Timestamp invalid, timestamp path: %s", str(ts_path))
    ...
    def _get_file_path_from_dir(self, dir_path, file_name):
        logger.debug(
            ...
            str(file_name), str(dir_path)
        )
        files = [f for f in os.listdir(dir_path) if file_name in f]
        if not files:
            logger.exception(
                "[validation] no file with given file name: %s was found in dir: %s",
                str(file_name), str(dir_path)
            )
            raise FileNotInDirectoryError("No files with given name in directory. name: %s", file_name)
    ...
    def _verify_certificate_with_crl(self, dir_path, crl_name, cert_name):
        ...
        validate_certificate(crl, os.path.join(dir_path, 'certificate_files', cert_name))
    ...
    def _verify_original_file(self, archived_file_hash, origin_path):
        logger.debug("[validation] verifying original file hash with archivated")
        if self.config["remote_access"] == False:
        ...
        logger.debug(
            "[validation] gathering origin file hash success full, verification..."
        )
        try:
    ...
    def _get_remote_file_hash(self, file_path):
        logger.debug(
            "[validation] creating sftp connection for getting origin file hash if it still same"
        )
        with closing(
            get_sftp_connection(self.config["remote_access"])
            ) as sftp_connection:
            logger.debug(
                "[validation] connection created successfully, getting hash of remote file"
                )
            try:
                return get_remote_hash(
                    sftp_connection, file_path, sha512
                )
            except Exception as e:
                logger.exception("Unable to get remote file digest", exc_info=True, stack_info= True)
                raise UnableToGetRemoteFileDigest("Unable to get remote file digest")

    def __get_file_name(self, path):
        head, tail = ntpath.split(path)
        logger.debug(
            "[validation] getting file name from path, splited path head: %s, tail(should be name) %s ",
            str(head), str(tail)
            )
