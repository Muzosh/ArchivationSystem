...
def make_task(config, task_message):
    c_maker = Connection_maker(config.get('rabbitmq_connection'))
...
def raise_system_exit():
    raise SystemExit(
   f"Usage: {sys.argv[0]} (-c | --config) <path to yaml config for Rabbitmq connection> (-fp | --filePath) <path to file which will be archivated> (-o | --owner) <owner name>"
    )
...
def parse_arguments(args):
    ...
    if args[4] == '-o' or args[4] == '--owner':
        owner = str(args[5])
    if not(isinstance(config_path, PurePath) or isinstance(archivation_file_path, PurePath) or isinstance(owner, str)):
        raise_system_exit()
...
def main():
    ...
    make_task(
        parsed_config,
        format_task_message(archivation_file_path, owner)
        )
